...
export default function Overview() {
  const { onOpenCreateModal, triggerToast } = useOutletContext();
        ...
        <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-xs">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-base font-bold text-gray-800">
              Recent Appointments Waiting Status Update
            </h3>
            <button
              onClick={() => navigate("/admin/appointments")}
              ...
              >
                <div>
                  ...
                  <p className="text-xs text-gray-500">
                    {app.doctorName} &bull; {app.departmentName} &bull;{" "}
        ...
        <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-xs">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {departments.map((dept) => {
              ...
              const receptionistCount = receptionists.filter(
                (r) => r.departmentId === dept.id,
              ).length;
              return (
                <div
                  key={dept.id}
                  className="p-4 rounded-lg border border-gray-100 hover:bg-gray-50/50 flex flex-col justify-between"
                >
                  <div>
                    <div className="flex items-center justify-between">
                      ...
                      <span
                        className={`h-2.5 w-2.5 rounded-full ${dept.isActive ? "bg-green-500" : "bg-gray-300"}`}
                      />
                    </div>
                  ...
                  <div className="mt-3 flex items-center justify-between text-xs text-gray-500 border-t border-gray-50 pt-2">
                    <span>
                      Doctors: <b>{doctorsCount}</b>
                    </span>
                    <span>
                      Staff: <b>{receptionistCount}</b>
                    </span>
                  </div>
                </div>
              );

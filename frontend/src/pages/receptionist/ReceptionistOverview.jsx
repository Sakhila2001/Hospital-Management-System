...
export default function ReceptionistOverview({ receptionist, triggerToast }) {
  ...
  const pendingTriageApps = appointments.filter((app) => app.status === "pending");
  ...
  useEffect(() => {
    if (selectedApp) {
      ...
      const initialDeptId = receptionist.departmentId || (activeDepts.length > 0 ? activeDepts[0].id : "");
  ...
  useEffect(() => {
    if (routedDeptId) {
      const match = availableDocs.filter((d) => Number(d.departmentId) === Number(routedDeptId));
          ...
          <table className="w-full text-sm text-left text-gray-500 min-w-[700px]">
            <thead className="text-xs text-gray-700 bg-gray-50/75 border-b border-gray-150">
              <tr>
                ...
                <th scope="col" className="px-6 py-3">Symptoms / Reason</th>
            ...
            <tbody className="divide-y divide-gray-100">
              {pendingTriageApps.map((app) => (
                <tr key={app.id} className="hover:bg-gray-50/50">
                  ...
                  <td className="px-6 py-4 capitalize font-semibold text-gray-550">{app.type.replace("_", " ")}</td>
                  <td className="px-6 py-4 text-gray-550 max-w-xs truncate" title={app.appointmentReason}>
        ...
        {selectedApp && (
          <form onSubmit={handleTriageSubmit} className="space-y-4">
            ...
            <div className="bg-gray-50 p-3 rounded-lg border border-gray-150 text-xs text-gray-600 space-y-1.5">
              <p><b>Patient:</b> {selectedApp.patientName}</p>
              <p><b>Preferred Slot:</b> {selectedApp.date} at {selectedApp.time}</p>
            </div>

            {/* Department */}
            <div>
              <label className="block text-[10px] font-bold text-gray-500 uppercase">Assign Clinical Department</label>
              <select
                value={routedDeptId}
            ...
            {/* Doctor */}
            <div>
              <label className="block text-[10px] font-bold text-gray-500 uppercase">Assign available Doctor</label>
              <select
                value={assignedDocId}
              ...
              >
                ...
                {filteredDocs.length === 0 && (
                  <option value="">No available doctors in this department</option>
                )}
              </select>
            </div>
            ...
            {/* Triage type */}
            <div>
              <label className="block text-[10px] font-bold text-gray-500 uppercase">Triage Category</label>
              <select
                value={triageType}
            ...
            {/* Reason */}
            <div>
              <label className="block text-[10px] font-bold text-gray-500 uppercase">Symptom details / Medical Notes</label>
              <textarea
                rows="3"
                value={triageReason}
